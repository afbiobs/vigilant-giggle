#!/usr/bin/env python3
"""Parse devotional DOCX files into per-day JSON outputs."""
...
class RobustThoughtProcessor:
    """A tolerant parser for devotional content with detailed diagnostics."""
    ...
    def classify_paragraph(self, raw_text: str, para_index: int, state: dict):
        for pattern, desc in self.day_patterns:
            ...
            if match:
                return "DAY_MARKER", ParseDiagnostic(para_index, raw_text, "DAY_MARKER", f"Matched day pattern: {desc}", matched_pattern=desc)

        matched, desc, _ = self._match_any_pattern(raw_text, self.study_text_patterns)
        if matched:
            return "STUDY_TEXT_HEADER", ParseDiagnostic(para_index, raw_text, "STUDY_TEXT_HEADER", f"Matched study text pattern: {desc}", matched_pattern=desc)

        matched, desc, _ = self._match_any_pattern(raw_text, self.prayer_patterns)
        if matched:
            return "PRAYER", ParseDiagnostic(para_index, raw_text, "PRAYER", f"Matched prayer pattern: {desc}", matched_pattern=desc)

        matched, desc, _ = self._match_any_pattern(raw_text, self.bible_reading_patterns)
        if matched:
            return "BIBLE_READING", ParseDiagnostic(para_index, raw_text, "BIBLE_READING", f"Matched Bible reading pattern: {desc}", matched_pattern=desc)

        if state.get('awaiting_title'):
            return "TITLE", ParseDiagnostic(para_index, raw_text, "TITLE", "First non-day paragraph after Day marker, interpreted as title")
        ...
        if state.get('awaiting_scripture_text'):
            return "SCRIPTURE_TEXT", ParseDiagnostic(para_index, raw_text, "SCRIPTURE_TEXT", "First paragraph after scripture reference, interpreted as scripture text/quote")

        return "DEVOTIONAL", ParseDiagnostic(para_index, raw_text, "DEVOTIONAL", "No special markers found, classified as devotional content")
        ...
        for issue in validation_report:
            ...
            for diagnostic in issue['diagnostics']:
                emoji = {
                    ...
                    "BIBLE_READING": "📚",
                    "DEVOTIONAL": "💭",
                }.get(diagnostic['classification'], "❓")
                lines.append(f"    {emoji} [{diagnostic['classification']}] Para #{diagnostic['para_index']}")
                lines.append(f"       Text: \"{diagnostic['text_preview']}\"")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Process devotional DOCX into JSON files")
    parser.add_argument("input_docx", help="Path to source .docx file")
    parser.add_argument("output_dir", nargs="?", default="data", help="Output directory for generated JSON files (default: data)")
...
def main():
    ...
    result = processor.process(args.input_docx, args.output_dir)

    print(result["report"])
    print()
    print(f"Output written to: {result['output_path']}")
    print("- Individual day files: day-NNN.json")
    print("- Master index: index.json")
    print("- Validation reports: validation_report.json, validation_report.txt")
